...
class POMCPWITHPPO(BeliefTreeSolver):
    ...
    def __init__(self, agent):
        ...
        :return:
        """
        super(POMCPWITHPPO, self).__init__(agent)
        self.logger = logging.getLogger('POMDPy.Simulation')
        self.logger.setLevel("INFO")
        self.fast_UCB = [[None for _ in range(POMCPWITHPPO.UCB_n)] for _ in range(POMCPWITHPPO.UCB_N)]
        ...
        self.A2CModel = PPOModel(state_dimension, state_space, action_dimension, action_space, self.model.DRLType)
        self.A2CSample = Sample(self.model.MaxNumIterationForBeliefNode)

        for N in range(POMCPWITHPPO.UCB_N):
            for n in range(POMCPWITHPPO.UCB_n):
                if n is 0:
    ...
    def find_fast_ucb(self, total_visit_count, action_map_entry_visit_count, log_n):
        ...
        if total_visit_count < POMCPWITHPPO.UCB_N and action_map_entry_visit_count < POMCPWITHPPO.UCB_n:
        ...
        if self.model.DRLType == DRLType.IS_PPOModel.value:
            if tree_depth != 0 :
                ...
                if sampleStatus :
                    ...
                    sampleStatus = self.A2CSample.add_batch_sample(belief_node.id, action_node.state_for_learning,
                                                                   action.UAV_deployment, action_node.old_logprob_v,
                                                                   reward, is_terminal)
