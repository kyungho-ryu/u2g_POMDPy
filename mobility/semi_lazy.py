...
class SLModel :
    ...
    def get_gmu_locIndex(self, id, k, *args):
        if k==1 :
            ...
            result = self.prediction_probabilistic_next_states(
                args[0][2], MConfig.theta, args[0][0], args[0][1], args[0][3], k)

        if result == [] :
            self.logger.info("{}' prediction depth is overed : {}".format(id, k))
            return -1, -1, -1, -1
    ...
    def get_reference_objects(self, id, backward_traj):
        ...
        # 5. LOOKUP PROCESS
        RO = self.tg.lookup(self.MOS[id].id, backward_traj)
    ...
    def prediction_probabilistic_next_states(self, RO, theta, t0Loc, S, eta, k) :
    ...
    def prediction_probabilistic_path(self, RO, theta, id) :
        ...
        if len(RO)<= 0 :
            ...
            S = State()  # state
            t0Loc = self.MOS[id].backward_traj[-1]
            while eta > theta and len(selectedPath) < MConfig.MaxPath:
                totalDensity = 0
                previousRO = []
